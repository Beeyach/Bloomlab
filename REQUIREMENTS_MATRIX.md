...
| REP-003 | No fake analytics or meaningless metrics anywhere in the product. Phase 15: `noFakeAnalytics.test.ts` scans every learner-facing module for a percentage or money amount written into markup and for a division handed straight to a percentage or currency formatter outside `reporting/`, and fails the build on either. It is a regression guard over those source shapes, not a proof about every possible implementation; REP-003 itself stays wider than the scan. A rate over a zero denominator is `null` and reads Not enough data (D-137). | P0 | all | PASSED | §70, §74 |
