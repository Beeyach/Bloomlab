...
| DES-009 | App shell: compact left rail ~68–80 px with Home, Campaign, Skill Map, Simulator, Clients, Portfolio, Playground; minimal top context; no giant sidebar. Phase 12: rail 80 px from one token with the page beside it at 768, 1024 and 1440; on phones a 64 px bar with four named areas and a labelled More opening the rest, every area named and reachable (D-116, `npm run review:rail`, `App.test.tsx`); Clients and Portfolio with Phases 23–24. REAL TABLET RAIL CHECK: PENDING. | P1 | 7 | PARTIAL | §73 |
