...
| DES-009 | App shell: expanded icon + label desktop/tablet sidebar and collapsed accessible icon-only rail, explicit toggle and persistent local device preference. Shared active-width token offsets content; one independent vertical destination scroller supports genuine wheel, keyboard/focus, scrollbar and tablet touch reachability at short heights without horizontal overflow. Phones retain the 64 px four-plus-More composition and Escape focus return. Fresh two-state evidence replaces the contradicted Phase 26 attestation. | P1 | 7 | IN_PROGRESS | §73, D-202 (supersedes D-117) |
