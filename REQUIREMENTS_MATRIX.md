...
| WFL-004 | Test contact: use existing simulated contact or generate one; run visibly through the workflow showing current node, current values, branch result, timeline events. Phase 12: existing or generated contact enrolled from the test panel, current node lit, travelling dot, resolved values, branch result and timeline rows from the engine (probe `run-and-replay`, 14 rows). | P0 | 12 | PASSED | §52 |
...
| WFL-012 | First Workflow Execution signature moment: watch the contact travel through the system. Phase 12: the run animates the contact along the graph with the timeline filling in order; Replay walks it again; reduced motion shows the end state at once (probe `run-and-replay`, `reduced-motion`). | P2 | 12 | PASSED | §161 |
...
| DES-009 | App shell: compact left rail ~68–80 px with Home, Campaign, Skill Map, Simulator, Clients, Portfolio, Playground; minimal top context; no giant sidebar. Phase 12: rail 80 px from one token with the page beside it at 768, 1024 and 1440; 64 px bar at 390 and 320; seven areas; Clients and Portfolio with Phases 23–24 (`npm run review:rail`). | P1 | 7 | PARTIAL | §73 |
