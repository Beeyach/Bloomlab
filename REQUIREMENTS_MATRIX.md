...
| PRD-004 | Substantially usable without runtime AI: curriculum, simulator, progression, exercises, deterministic grading, CRM/Workflow/Funnel Labs, pricing drills, portfolio and saved progress all work with AI Off. | P0 | all | PASSED | §106, §107, §143, TA§1 |
...
| PRD-009 | Architecture avoids dead ends that would force a rewrite of the learning engine for later commercialisation, without building commercial infrastructure now. | P1 | all | PASSED | §3, TA§1 |
...
| EXR-006 | RUN THE LEAD: learner predicts execution of a contact through a workflow, then actual execution is animated. Phase 12: the prediction is captured and the real run is graded; the animation is the Lab's Replay rather than inside the runner page. | P1 | 9 | PASSED | §27 |
...
| SIM-011 | Realistic failures simulated: missing phone, DND, invalid webhook auth, missing field, unavailable appointment, duplicate enrollment, bad condition, workflow loop, integration failure. Observable symptoms shown before fixes. Phase 15: nine incident scenarios in `content/scenarios/SC-glowhaus-incident-*.yaml` reproduce each failure from real engine behaviour, none of it special-cased for the lesson; the Incident Room shows the symptom, the client's complaint, the logs and the system state, and never names the fault. Pinned by nine regression fixtures (`PHONE-001`, `DND-001`, `WEBAUTH-001`, `FIELD-001`, `SLOTS-001`, `REENTRY-001`, `CONDITION-001`, `LOOP-001`, `INTEGRATION-001`). | P1 | 15 | PASSED | §49 |
...
| FUN-004 | Funnel Autopsy data inspection: traffic source, conversion rate, scroll behavior, form completion, booking rate, drop-off. Phase 15: the same six views, from the same `funnelAutopsy` projection, inside the Funnel Lab's SIMULATE mode. A funnel with no traffic says so rather than showing zeroes. | P1 | 15 | PASSED | §27 |
...
| CONV-002 | Written sales simulation: persistent inbox-like conversations where client messages react to the learner's answer; conversation continues naturally instead of always showing "Correct." | P1 | 16 | PASSED | §120 |
...
| ID | Requirement | Priority | Phase | Status | Spec |
|---|---|---|---|---|---|
| REP-001 | Reporting Lab calculates from actual simulator data: leads, conversion, booking rate, show rate, close rate, revenue, pipeline value, source performance, response rate, time to contact. Phase 15: `SC-glowhaus-reporting` holds three weeks as 269 scheduled events and no totals; Run the window advances the clock through the ordinary execution door and every metric is a projection over the resulting log. 22 tests reconcile each of the ten metrics against the events behind it. | P1 | 15 | PASSED | §60 |
| REP-002 | Reporting trains diagnosis (bottleneck reasoning), not passive dashboards. Phase 15: the Lab opens on the stage chain, every metric shows its calculation and the events behind it, and `EX-FIX_IT-glowhaus-reporting-bottleneck` grades the diagnosis: naming show rate passes, naming close rate is a critical failure. | P1 | 15 | PASSED | §60, §18 |
| REP-003 | No fake analytics or meaningless metrics anywhere in the product. Phase 15: `noFakeAnalytics.test.ts` scans every learner-facing module for a percentage or money amount written into markup and for a reporting rate computed outside the projection, and fails the build on either. A rate over a zero denominator is `null` and reads Not enough data (D-137). | P0 | all | PASSED | §70, §74 |

## SAL — Sales and Delivery

| ID | Requirement | Priority | Phase | Status | Spec |
|---|---|---|---|---|---|
| SAL-001 | Evidence-based audit: every finding classified VERIFIED (directly observed) / LIKELY (evidence suggests) / UNKNOWN (needs confirmation); unsupported claims penalised. | P0 | 16 | PASSED | §20 |
| SAL-002 | Prospect qualification: some prospects should be skipped; ICP fit, economics, recurring need, technical fit, decision-maker access. | P1 | 16 | PASSED | §20 |
| SAL-003 | Cold email training: opener, evidence, relevance, problem, CTA, follow-up. | P1 | 16 | PASSED | §20 |
| SAL-004 | Discovery structure: opening, agenda, current process, desired result, pain, volume, impact, urgency, tools, staff, stakeholders, budget, decision process, next step; plus technical discovery. | P1 | 16 | PASSED | §20 |
| SAL-005 | Listening graded: excessive talking and premature pitching penalised. | P1 | 16 | PASSED | §20 |
...
| DES-009 | App shell: user-resizable expanded horizontal icon + label desktop/tablet sidebar (200 px default, 176–280 px with viewport protection) and fixed 76 px collapsed accessible icon-only rail. Accessible pointer/keyboard separator, reset, explicit toggle and persistent local device mode/width preferences. Shared active-width token offsets content; one independent vertical destination scroller supports genuine wheel, keyboard/focus, scrollbar and tablet touch reachability at short heights without horizontal overflow. Phones retain the 64 px four-plus-More composition and Escape focus return. Fresh two-state evidence replaces the contradicted Phase 26 attestation. | P1 | 7 | IN_PROGRESS | §73, D-202 (supersedes D-117) |
...
| RSP-004 | Mobile recompositions: Workflow → vertical step editor; CRM → stage view / local horizontal scroller; Academy → editorial reading; Call Room → mobile-first voice; Inbox → natural conversation flow; Skill Map → territory-first. Phase 13: Funnel → the step list and the block inspector become labelled sheets on phones and every mode stays operable at 390 and 320 (probe). Phase 12: Workflow → vertical step editor holds at 390 and 320 (probe). Call Room and Inbox recompositions belong to Phase 21 and later. | P1 | 12 | PASSED | §83 |
...
| ID | Requirement | Priority | Phase | Status | Spec |
|---|---|---|---|---|---|
| A11Y-001 | Full keyboard operability of core flows. Phase 13: the Funnel Lab's reorder is reached by Tab with a visible ring and activated by Enter — there is no drag-only path anywhere in it (probe `keyboard-reorder`). | P0 | 2 | PASSED | §84 |
...
| DATA-006 | R2 holds generated audio, voice assets, screenshots, portfolio media, fieldwork media, recovery backups, scenario attachments; D1 holds metadata; no huge binaries or giant JSON blobs in D1. | P1 | 20 | PASSED | §94, TA§15 |
...
| INF-011 | Error boundaries per major environment: Claude failure never breaks Workflow Lab, Call Room failure never breaks CRM, sync failure never destroys local state. No external service can destroy study progress. | P0 | 7 | PASSED | §149, TA§79 |
