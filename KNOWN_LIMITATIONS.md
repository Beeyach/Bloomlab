...
- **No sync transport yet (Phase 4).** The outbox fills and coalesces but nothing drains it, so the indicator never reaches "Synced"; the device label and the provisional `local:` learner id are local-only until the Bloomlab Sync Key exists. Records written now carry the full envelope and will sync unchanged.
