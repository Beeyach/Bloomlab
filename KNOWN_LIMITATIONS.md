...
- **Holo physics observed live** at 800 px and 768 px viewports, on the dev server and on the production bundle (`vite preview`): tilt ≈ 5.2–5.5° at a corner, glare, band sweep, hue shift, rim, shadow, follow easing and settle all respond to a real pointer; touch press/drag/release verified with pointer events. The review tool only renders frames while an input action runs, so eased values were read across successive pointer moves; a normal browser runs the loop at display rate. Reduced motion was verified by forcing the tokens the media query sets (the machine's OS preference could not be toggled from the review tool); the hook path is unit-tested.
