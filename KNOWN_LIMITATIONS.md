...
- CALL-003 is PARTIAL: the human proposal reached Closing but final feedback failed twice. The saved four-turn attempt is recoverable. A bounded fictional reproduction found missing rubric items, then an unsupported learner quotation in the single repair. Provider output now requires all eight named dimensions and receives the specific repair cause; existing grading and citation validators remain mandatory. Deployed fix acceptance and recovery of the saved human result are pending. The old failed responses were not retained, so their exact historical validation subtype cannot be recovered. See `docs/reviews/phase-21-feedback-reliability.md`.
