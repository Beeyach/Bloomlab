...
- **SAL-009** Proposal exercise requires all eight sections.
  - **PASSED (Phase 17).** `EX-WRITE_IT-summit-proposal` asks for problem, recommendation, scope, price, timeline, assumptions, exclusions and acceptance as eight separate answers, each with its own question, reader and cap. The schema refuses a proposal missing any of them and refuses two fields claiming the same section, so a half-proposal cannot be authored.
