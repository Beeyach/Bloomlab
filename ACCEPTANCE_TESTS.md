...
- **DES-009** At 1440/1024/768, expanded sidebar defaults to 200 px and has horizontal icon + label rows at every width from 176–280 px; collapsed rail stays 76 px with accessible icon-only destinations, hover/focus tooltips, non-colour-only active state and keyboard/touch toggle. Measured expanded icon bounds must sit left of a visible label with centerlines within 2 px and 44–48 px row height. Collapsed labels must have zero rendered boxes; visible icons must be centered within 1 px in targets at least 44 px. Inspect actual captures at all three desktop/tablet widths in both states. Both share the active page-offset value. Drag the named, focusable right-edge separator smaller/larger in real time; verify matching content offset, no width animation, no content underneath and no per-move storage writes. Verify Left/Right 8 px, Home minimum, End effective maximum, exposed separator values and visible focus; double-click resets to 200 px. Mode and width persist locally across reload and collapse/expand; invalid/missing width falls back to 200, out-of-range values clamp, and blocked storage retains session operation. At 768, clamp expanded maximum to 232 to preserve 536 px content and the readable Workflow canvas without overwriting a wider saved preference. Capture and inspect minimum/default/maximum at 1440/1024 and constrained 768; important destination names remain understandable at minimum. Genuine wheel and small trackpad-equivalent deltas must reveal previously offscreen lower destinations at 720/600/480 px wherever overflow exists; native scrollbar drag, forward/reverse Tab focus visibility and tablet touch must work. Page wheel input remains independent; boundary gestures have deliberate native chaining. No horizontal overflow or content loss; reduced motion works. At 390/320 retain the 64 px four-plus-More bar, labelled reachable destinations and Escape focus return, with no desktop toggle or resize handle. Fresh exact-head CI and deployed Worker/browser identity and interaction evidence supersede D-117/Phase 26; Chromium emulation does not certify physical Safari.
...
- **CUR-032** `advancedPaths.test.ts` enforces all seven named post-Field-Ready paths, only Bloomwired recommended, existing skill IDs and learning/practical coverage; mutations remove paths, foundations, recommendations or coverage and must fail. The screen tests cover loading, storage error/retry, unknown bookmarks, all choices and shared evidence without awarding Field Ready. `advanced-paths-probe.mjs` checks paths/directory/Skill Map at 1440/1024/768/390/320, bookmark reload, actual exercise evidence shared across paths, unfinished Field Ready, keyboard focus, touch, offline and reduced motion. No duplicate skill or progress store.
...
- **PAY-001** Core tests verify products/prices, invoices/links/subscriptions, failed/retried/duplicate/refunded charges, workflow event adapters, replay and revenue. Browser coverage includes offline renewal, reload and the account-backed practical. Presentation tests cover loading, storage error, refusal and empty catalogue. All nine changed Lab surfaces plus four lessons and four account-backed runner work areas are checked at 1440/1024/768/390/320 with AI Off, visible keyboard focus, touch and reduced motion; the Phase 24 short-height rail remains required.
