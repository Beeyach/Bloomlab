...
- **DES-009** At 1440/1024/768, expanded sidebar has horizontal icon + label rows (220–248 px) and collapsed rail has accessible icon-only destinations (68–80 px), hover/focus tooltips, non-colour-only active state and keyboard/touch toggle. Measured expanded icon bounds must sit left of a visible label with centerlines within 2 px and 44–48 px row height. Collapsed labels must have zero rendered boxes; visible icons must be centered within 1 px in targets at least 44 px. Inspect actual captures at all three desktop/tablet widths in both states. Both share the active page-offset token and persist locally across reload. Genuine wheel and small trackpad-equivalent deltas must reveal previously offscreen lower destinations at 720/600/480 px wherever overflow exists; native scrollbar drag, forward/reverse Tab focus visibility and tablet touch must work. Page wheel input remains independent; boundary gestures have deliberate native chaining. No horizontal overflow or content loss; reduced motion works. At 390/320 retain the 64 px four-plus-More bar, labelled reachable destinations and Escape focus return, with no desktop toggle. Fresh exact-head CI and deployed Worker/browser identity and interaction evidence supersede D-117/Phase 26; Chromium emulation does not certify physical Safari.
