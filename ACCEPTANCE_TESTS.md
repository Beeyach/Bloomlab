...
Phase 21 evidence (2026-09-08): `docs/reviews/phase-21-call-room.md`, `docs/operations/call-room.md`, focused browser/Worker tests and `scripts/review/call-probe.mjs`. CALL-001/CALL-004 local interface acceptance passes. Other Phase 21 targets are IMPLEMENTED_UNVERIFIED: native browser/IndexedDB tests and explicit provider fixtures do not substitute for the live preview microphone/Google/IAM/dynamic TTS checklist. Preview now has the Google `secret_text` and enabled call gates. Supplementary live prerecorded-audio diagnostics verify Google recognition, private R2 recovery/deletion and AI grading; actual microphone/full phone and keyboard calls and dynamic TTS/cache success remain outstanding. Production stays gated. No claim of complete Phase 21 acceptance is made.
