...
- **REP-003** (cross-cutting) No fabricated number reaches a learner-facing screen.
  - **PASSED (Phase 15).** `noFakeAnalytics.test.ts` scans every learner-facing module for a percentage or money amount written into markup and for a division handed straight to a percentage or currency formatter outside `reporting/`, and fails on either. Those are the shapes the mistake has taken here; the scan guards against them recurring rather than proving no other form exists. Flag-gated developer surfaces are excluded by name. The two hits it found while being written were a bar-width ratio in the Academy diagram (kept: it is arithmetic about pixels) and the Reporting Lab dividing leads by visits (fixed by carrying `conversion` on the source row).
