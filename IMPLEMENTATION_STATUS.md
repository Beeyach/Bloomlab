...
Phase 8 — Academy: **complete, with the shared-simulator embed host waiting for Phase 10** — the three authored learning units render at `/academy/:unitId` from their MDX bodies, compiled at build time into one lazy chunk each (the client parses no MDX): typography-led sections with a contents list, callouts, native expandable depth, registry-backed technical notes on real GHL features, practice pointers to authored exercises, two Bloomlab-drawn diagrams (a funnel from the unit's four rates; a workflow path from the scenario that defines it) and a deterministic funnel-math interactive whose sliders recompute counts, revenue and the biggest leak. "Finish this unit" is the only thing that records: exposure evidence for the unit's skills through `recordEvidence`, UNSEEN → LEARNING and never further, no duplicate on re-reading, written locally offline and synced later. Continue, session-plan items and the capability sheet open a unit directly when the engine's next step is a unit; when it is an exercise the screens say the runner arrives with Phase 9. Verified by 14 Academy tests over the real bundle, the five-width capture audit of all three units, and the Academy probe in Chrome (arrow keys on the slider, Enter on the disclosure and Finish, the evidence row, reload, a second unit opened and finished offline then kept across an offline reload, reduced motion, touch at 390 px). The `<Simulation>` embed states what will run and simulates nothing (CUR-036 PARTIAL). Phase 9 — Exercise Runner is waiting for the go-ahead.
...
- PRD-003 — evidence: gates resolve on evidence counts and kinds (campaign test 9); a quiz never changes the ladder; an independent exercise does (web test). Phase 8: finishing a unit records exposure only — UNSEEN → LEARNING, never PRACTICED or INDEPENDENT; repeated exposures and passed quizzes leave the skill at LEARNING with zero independent demonstrations (`academy.test.tsx`).
...
Phase 9 — Exercise Runner targets: the core shell and deterministic grading (EXR-*), the first families (Build It, Fix It, Run the Lead, Edge Case, Architecture Decision, Rebuild Blind), attempts and evidence through `recordEvidence` with hints rolled up to assistance, and the Academy practice pointers and Command Center steps becoming runnable; not started until the go-ahead.
...
311 requirements registered · 86 PASSED · 8 IN_PROGRESS · 14 PARTIAL · 0 BLOCKED · 2 DEFERRED · 201 NOT_STARTED. Run the validator for the live count by status and priority.
