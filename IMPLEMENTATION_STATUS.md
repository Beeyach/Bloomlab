...
- HOL-003 — evidence: live pointer at a card corner gave `nx 0.903 / ny −0.859`, rotateX −5.2° / rotateY −5.5° (≤ 6°), band position 95%/7%, glare translated toward the pointer with opacity 0.75, rim at the pointer angle (46°), shadow offset away from the pointer. Smoothing is a frame loop (D-022) verified in the production bundle (`vite preview`, preview environment, 800 px and 768 px): the pose eases toward the pointer (≈ 120 ms), eases back to neutral on leave (≈ 95 % at 420 ms) and `data-tracking` clears only once converged; the review tool only composites frames during input actions, so convergence was read across successive pointer moves rather than a wall-clock wait. Unit tests cover the maths, clamping, follow, settle and reset.
