...
- DATA-001 — evidence: the full chain runs end to end: a note written on one device (UI → Dexie row + outbox in one transaction) is pushed by the background scheduler, accepted by the Worker into D1, and pulled by the other device (`npm run review:sync`, local preview and deployed preview); nothing in the UI waits on the network, and the Phase 3 offline evidence still holds.
...
- SYNC-007 — evidence: every synced record carries the six fields plus `created_at`; the outbox coalesces repeated pending changes per record (a keystroke stream or a drag becomes one operation) and the Worker rejects writes for another device; `sync_operations` logs only accepted changes.
