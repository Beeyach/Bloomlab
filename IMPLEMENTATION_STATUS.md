...
Phase 7 — Command Center + Skill Map: **complete for the areas that exist** — the app shell has its compact rail (72 px column from 768 px, a bottom bar on phones) with Home, Campaign and Skill Map plus the flagged developer surfaces; the home route is the Command Center, which answers "what should I do next?" from the learner's real evidence (a live query over the Phase 6 engine): a holographic continuation object (campaign, gate, the next required capability, its state in words, the engine's next step and the gate's capability count), Build my session on the real session builder with 30 min / 1 hour / 2 hours / Deep Session and Continue, due retrieval, capabilities that need another run, work-ahead capabilities with their gates, recent evidence in the §159 words, and the learner's focus. `/campaign` shows Field Ready as competency gates with pass criteria and per-capability state. `/skills` is the Skill Map: ten HoloMaterial territory objects around JUDGMENT counting demonstrated capabilities, a territory panel of SkillCards whose material follows the mastery state, availability in words (Locked with the prerequisite names, Available, Next required, Work ahead, In progress, Demonstrated, Needs refresh), and a capability sheet (side sheet on desktop, bottom sheet on phones) with state, prerequisites, what it opens, the next step with unit or exercise text, evidence history, review due and Set as focus. Every route sits in a screen error boundary so a failing screen leaves the shell and local data intact. Verified by 13 screen tests over the real content bundle, the five-width capture audit, the holographic probe on the Judgment object (mouse follow, settle, reduced motion, touch press–drag–release), keyboard and touch probes, and the deployed preview. DES-009 and DES-010 stay PARTIAL only for rail areas and the active-client block that belong to later phases; no metric, state or step on these screens is invented. Phase 8 — Academy is waiting for the go-ahead.
...
- RSP-003 — cross-cutting: nothing removed on mobile so far. Phase 7: every Command Center section, every gate and every territory, capability and sheet action is present at 320 px.
...
- PERF-001 — every screen is a route-level chunk (`CommandCenter`, `CampaignScreen`, `SkillMap`, `SyncScreen`, `SystemDiagnostics`, `DesignGallery`) over a shared `learning` chunk; the production build lists them separately. Heavy simulators do not exist yet, so their lazy loading is proven with them.
- RSP-004 — Skill Map → territory-first: a single column of territory objects with JUDGMENT central, the selected territory's capabilities beneath, a bottom sheet for the capability, verified at 390 / 320 by the capture audit and the touch probe. The other five recompositions belong to Phases 11–21.
- A11Y-001 — proven for every flow that exists: every interactive element is a native button, link, input, select, textarea or dialog; keyboard tests for Button, SkillCard, Popover (Escape), Sheet (cancel); Tab reaches controls in the running app. Phase 7: the keyboard probe tabs through the rail, sync status, continuation object, session chips, plan links and rows on the Command Center, every gate link on the Campaign, every territory object and card on the Skill Map, and the sheet (focus trapped, Escape closes it and focus returns to the card). Remaining before PASSED (the acceptance names flows that do not exist yet): starting a session and running its first item from the keyboard (the unit and exercise runtimes, Phases 8–9), opening an exercise and submitting it (the exercise runner, Phase 9), and navigating the labs — CRM Lab (Phase 11), Workflow Lab with its non-drag alternatives (Phase 12), Funnel Lab (13), Calendar Lab (14), Pricing Arena (17) and the Call Room (21). Each of those phases must add its keyboard probe and tests before the requirement can be marked PASSED globally.
...
Phase 8 — Academy targets: interactive learning units rendered from the MDX content with embedded small simulations (ACA-*), exposure evidence recorded through `recordEvidence`, unit reading as the first real "Continue" destination; not started until the go-ahead.
...
311 requirements registered · 85 PASSED · 8 IN_PROGRESS · 13 PARTIAL · 0 BLOCKED · 2 DEFERRED · 203 NOT_STARTED. Run the validator for the live count by status and priority.
