...
| Final grading | One completed API call returned HTTP 200, score 82, all eight rubric items and `call_feedback` usage. Changing arbitrary browser submission text returned the identical saved rubric run, supporting server-confirmed reconstruction. One `questions` explanation credited the client's “Which part of following up a replacement quote do you want to understand?” to the learner. Score 82 is provider-completion evidence, not grading-quality acceptance. Deterministic critical/required override remains covered by exact-head tests; no separate favorable-model critical-failure live case was purchased. |
...
| Limitations | Real microphone/local queue/visible transcript acceptance, full touch/keyboard voice calls, successful dynamic TTS/cache, physical Safari/iOS and exact IAM-role inspection remain unverified. The observed feedback speaker-misattribution also remains unresolved. Existing abandoned-recording retention and uncertain-provider reconciliation limitations remain. |
