...
import { resetFunnelRun } from './commands';

/**
 * The Funnel Lab's run (D-096, D-108, D-109).
 *
 * The same rule every other Lab follows: one saved simulator run of the chosen scenario, resumed
 * rather than restarted, chosen by `currentRunId`, so the CRM Lab, the Workflow Lab and the Funnel
 * Lab opened on one scenario are all in the same account. Every change goes through `perform`,
 * which adopts an execution result — a success replaces the run, a refusal keeps it exactly as it
 * was and surfaces the reason.
 *
 * This is deliberately the same shape as `useWorkflowRun`. Phase 13 adds no second way to open,
 * resume, reset or switch a run.
 */

export const DEFAULT_FUNNEL_SCENARIO_ID = 'SC-glowhaus-funnel';

export interface FunnelRunState {
  run: StoredRun | null;
...
export interface FunnelRunApi extends FunnelRunState {
  perform: (
...
export function useFunnelRun(scenarioId: string = DEFAULT_FUNNEL_SCENARIO_ID): FunnelRunApi {
  ...
  const reset = useCallback(async () => {
    ...
    setRefusal(null);
    try {
      setRun(await resetFunnelRun(scenario, run.state.run_id));
    } catch (error) {
