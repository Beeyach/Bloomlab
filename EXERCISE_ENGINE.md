...
**Implementation (Phase 12, grader `2026.09.07-r1`, D-113):** every assertion is scored under one of `correctness · edge_cases · architecture · maintainability · explanation`. `dimensionOf` uses the authored `dimension` when the content names one, else architecture for architecture checks, edge cases for negative checks, explanation for state checks on a learner root (`decision`, `prediction`, `answer`), correctness otherwise. The score is the weighted share across dimensions that have at least one required or quality check, with weights from the exercise's `grading.weights`; a dimension with no checks is left out and the remaining weights are renormalised. Bonus checks stay outside the denominator. A failed critical check fails the attempt at any score. `GradeReport.dimensions` carries weight, total and passed per dimension (null when the exercise authors no weights) and the result view shows them. Workflow exercises are graded from the learner's own run through the Workflow Lab runtime (D-112): `state`, `events`, `references` and `architecture`, the last being the workflows the learner created or changed, normalised without positions.
